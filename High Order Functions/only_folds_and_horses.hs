...
boomBang :: (Integral a, Show a) => [a] -> [[Char]]
boomBang = foldr (\x acc -> if x `mod` 15 == 0 then "boomBang" : acc else if x `mod` 3 == 0 then "boom" : acc else if x `mod` 5 == 0 then "bang" : acc else show x : acc) []